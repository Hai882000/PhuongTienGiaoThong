...
public class QuanLyPhieuThue extends javax.swing.JPanel {
    ...
    private void btnXuatPhieuActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnXuatPhieuActionPerformed
        ...
        try {
            ...
            PhieuThueDao dao = new PhieuThueDao();
            List<PhieuThue> list = dao.findByKh(txtMaKH.getText(), txtNgayThang.getText());
    ...
    private void btnLapPhieuActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnLapPhieuActionPerformed
        ...
        try {
            ...
            List<PhieuThue> list = dao.findAll();
            for (PhieuThue phieuThue : list) {
                if(phieuThue.getBienSo().equalsIgnoreCase(txtBienSo.getText())){
                    MessageDialogHelper.showConfirmDialog(parentForm, "Phiếu thuê không được lưu do xe đã được thuê", "Cảnh báo");
                    return;
                }
            }
            ...
            for(ThongTinXe thongtinxe : ttx){
                if(thongtinxe.getBienSo().equalsIgnoreCase(txtBienSo.getText())){
                    KTBS = true;
                }
            }
            ...
            for(NguoiDung nguoidung : nd){
                if(nguoidung.getMaND().equalsIgnoreCase(txtMaNV.getText())){
                    KTNV = true;
                }
            }
            if(KTBS == true){
                if(KTKH == true){
                    ...
                    MessageDialogHelper.showConfirmDialog(parentForm, "Nhân viên không tồn tại", "Cảnh báo");
                }
                }else{
                    MessageDialogHelper.showConfirmDialog(parentForm, "Khách hàng không tồn tại", "Cảnh báo");
                }
            }else{
                MessageDialogHelper.showConfirmDialog(parentForm, "Biển số không tồn tại", "Cảnh báo");
            }
    ...
    private void btnTimActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnTimActionPerformed
        ...
        {
            PhieuThueDao dao = new PhieuThueDao();
            PhieuThue pt = dao.findById(txtMaPhieuTK.getText());
            if(pt !=null)
            {
                txtMaPhieu.setText(pt.getMaPT());
                txtNgayThang.setText(pt.getNgayThang());
